...
```{r}
#| echo: false
# Running TidyVerse library in the code due to mutate() not working properly without running it in the Console before each run. 
library(tidyverse)
```
...
PlaceboDF <- PlaceboDF |> mutate(PLBPUnder120 = PlaceboAfterBP < 120)
